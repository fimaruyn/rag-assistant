...
def report(
    ...
    min_missing_share: float = typer.Option(0.1, help="Порог доли пропусков для выделения проблемных колонок (от 0 до 1)."),
) -> None:
    """
    ...
    # Добавляем информацию о пороге пропусков
    quality_flags["min_missing_share"] = min_missing_share
    quality_flags["problematic_missing_columns"] = (
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Порог для проблемных колонок по пропускам: **{min_missing_share:.0%}**\n")  # Новая информация
        f.write(f"- Колонки с высокой долей пропусков: **{len(quality_flags['problematic_missing_columns'])}**\n")  # Новая информация
        ...
        if missing_df.empty:
            ...
            if quality_flags["problematic_missing_columns"]:
                f.write(f"**Проблемные колонки** (доля пропусков > {min_missing_share:.0%}):\n")
                f.write("\n".join([f"- `{col}`" for col in quality_flags["problematic_missing_columns"]]) + "\n\n")
        ...
        f.write(f"Гистограммы для первых **{max_hist_columns}** числовых колонок:\n")
    ...
    typer.echo(f"- Параметры отчёта: top-k категорий = {top_k_categories}, порог пропусков = {min_missing_share:.0%}")
