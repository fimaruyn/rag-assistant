...
# ---------- /summary-from-csv: возвращает детальную JSON-сводку по датасету ----------

@app.post(
    "/summary-from-csv",
    response_model=dict,
    tags=["summary"],
    summary="Получить JSON-сводку по CSV-файлу",
    description=(
        "Эндпоинт принимает CSV-файл и возвращает детальную JSON-сводку по датасету, "
        "включая информацию по каждой колонке: типы данных, пропуски, уникальные значения, "
        "статистики для числовых признаков."
    ),
)
async def summary_from_csv(file: UploadFile = File(...)) -> dict:
    """
    Эндпоинт, который принимает CSV-файл и возвращает полную JSON-сводку,
    аналогичную той, что генерируется в CLI-режиме с опцией --json-summary.
    """
    start = perf_counter()

    # Проверка типа файла (мягкая, как в других эндпоинтах)
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    # Генерация сводки с помощью существующей функции из core.py
    summary = summarize_dataset(df)
    ...
    # Преобразование сводки в JSON-совместимый формат
