...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # НОВАЯ ЭВРИСТИКА 1: Проверка на константные колонки
    has_constant_columns = False
    for col in summary.columns:
        # Считаем колонку константной, если есть хотя бы одно непустое значение
        # и все непустые значения одинаковые
        if col.non_null > 0 and col.unique == 1:
            has_constant_columns = True
            break
    flags["has_constant_columns"] = has_constant_columns

    # НОВАЯ ЭВРИСТИКА 2: Проверка на категориальные признаки с высокой кардинальностью
    has_high_cardinality_categoricals = False
    for col in summary.columns:
        # Рассматриваем только нечисловые колонки с достаточным количеством данных
        if (not col.is_numeric and 
            col.non_null > 100 and  # достаточно данных для анализа
            col.unique > 50):       # порог высокой кардинальности
            has_high_cardinality_categoricals = True
            break
    flags["has_high_cardinality_categoricals"] = has_high_cardinality_categoricals

    # Обновленный расчет интегрального показателя качества
    score = 1.0
    score -= max_missing_share  # чем больше пропусков, тем хуже
    
    # Штрафы за структурные проблемы
    ...
    if has_constant_columns:
        score -= 0.15  # константные колонки почти бесполезны
    if has_high_cardinality_categoricals:
        score -= 0.1   # высокая кардинальность может вызвать проблемы в ML
    
    # Ограничение скоринга в диапазоне [0.0, 1.0]
